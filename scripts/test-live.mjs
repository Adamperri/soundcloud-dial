...
if (!process.argv.includes("--controls")) throw new Error("Pass --controls to allow the live playback/volume test.");
...
const info = {
  ...
  plugin: { version: "1.0.0.0", uuid: "com.adamperri.soundclouddial" },
...
try {
  ...
  if (initialVolume !== undefined && /^\d+%$/.test(display.level) && Number.parseInt(display.level) !== initialVolume) {
    send("dialRotate", { ticks: (initialVolume - Number.parseInt(display.level)) / 2, pressed: false });
    await delay(1000);
  }
  if (initialPlayback && display.playback !== initialPlayback) { send("dialDown"); await delay(1000); }
  send("willDisappear");
  await delay(300);
  socket.close();
  server.close();
