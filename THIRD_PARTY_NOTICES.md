...
- Elgato Stream Deck SDK, schemas and utilities: MIT, Elgato/Corsair.
...
Source licenses are included under `licenses/` or copied from the locked npm packages.
Build-only dependencies are listed in `package-lock.json` and are not shipped as executables.

No SoundCloud songs, third-party album covers or personal Stream Deck profiles are distributed.
